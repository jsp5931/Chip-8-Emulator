...
function loadRom(rom,emuSpeed) {
    Module.ccall("loadRom", null, ['string', 'number'], [rom,emuSpeed]);
}
function startGame() {
...
Module["onRuntimeInitialized"] = function () {
    ...
    games.forEach(game => {
        game.addEventListener('click', function (event) {
            ...
            game.id = 'active'
            let speed = determineSpeed(game.innerHTML);
            console.log(speed)
            setInstructions();
            loadRom(selected,speed);
            startGame();
        })
    })
    ...
    resetButton.addEventListener('click', function(event){
        ...
        stopGame();
        let speed = determineSpeed(activeGame.innerHTML);
        loadRom(currentGame, speed);
        startGame();
    })
}
...
function setInstructions() {
    ...
    switch (activeGame.innerHTML) {
        ...
        case ("Tapeworm"):
            ...
            break;
        case ("Space Invaders"):
            document.querySelector('.instructions').innerHTML = "<p>1: BEGIN GAME<br>W: SHOOT<br>Q: MOVE LEFT<br>E: MOVE RIGHT</p>"
            break;
        case ("UFO"):
            document.querySelector('.instructions').innerHTML = "<p>Q: SHOOT LEFT<br>W: SHOOT MIDDLE<br>E: SHOOT RIGHT</p>"
            break;
        case ("Soccer"):
            document.querySelector('.instructions').innerHTML = "<p>1: MOVE UP<br>Q: MOVE DOWN</p>"
            break;
